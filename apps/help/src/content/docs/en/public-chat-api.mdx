...
The unprefixed paths `/public/agents/{embedToken}/...` predate versioning. They serve the
version 1 routes and fields that existed before versioning, and stay available for as long
as version 1 is served. They do not serve the MCP App card HTML route, and their
configuration response has no `bannerText`. Use the `v1` paths in new integrations.
...
carries one named error event with a plain text `data` line, then the connection closes:
...
A failed generation sends the `error` JSON event, then this named error event, then closes.
