...
A server you connected has a single action: the **trash icon** on its right. There
is no edit or overflow menu — connect and delete are the only actions. The built-in
**PDF export** server (see below) has no trash icon and can't be deleted.
...
[**Orchestration**](/en/agent-orchestration) and [**Embed**](/en/agent-embed). When
the platform is configured for it, every workspace has at least the built-in
**PDF export** server, so this tab is always there once the feature is enabled.
