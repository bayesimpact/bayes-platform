...
Un serveur que vous avez connecté a une seule action : l'**icône corbeille** à sa
droite. Il n'y a ni modification ni menu d'options — connecter et supprimer sont les
seules actions. Le serveur intégré **PDF export** (voir ci-dessous) n'a pas d'icône
corbeille et ne peut pas être supprimé.
...
[**Intégration**](/fr/agent-embed). Lorsque la plateforme est configurée pour cela,
chaque espace de travail dispose au moins du serveur intégré **PDF export**, donc cet
onglet est toujours présent dès que la fonctionnalité est activée.
...
- **Un agent n'utilise plus les outils d'un serveur** — vérifiez que le serveur est
  ...
  tous les agents).
- **Le lien de téléchargement indique qu'il a expiré** — demandez à l'agent de
  relancer l'export.
