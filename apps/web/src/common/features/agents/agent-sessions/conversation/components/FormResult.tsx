...
export function FormResult({
  ...
  assert(agent.fillFormEnabled, "FormResult: fillForm is not enabled on this agent")
  assert(agent.outputJsonSchema, "FormResult: Missing outputJsonSchema for fillForm agent")
  ...
  return (
    <Item className="absolute inset-0 flex-col items-stretch flex-nowrap gap-0 p-0">
      <ItemHeader className="basis-auto shrink-0 px-4 pt-4 pb-2">
        <ItemTitle className="text-lg">{t("conversationAgentSession:props.result")}</ItemTitle>
      </ItemHeader>
