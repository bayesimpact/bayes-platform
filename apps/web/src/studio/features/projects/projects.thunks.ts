...
export const addProjectAgentSessionCategory = createAsyncThunk<
  ProjectAgentSessionCategory,
  { name: string; assignToAllConversationalAgents: boolean },
  ThunkConfig
>(
  "projects/addProjectAgentSessionCategory",
  async ({ name, assignToAllConversationalAgents }, { extra: { services }, getState }) => {
    const state = getState()
    const organizationId = getCurrentId({ state, name: "organizationId" })
    const projectId = getCurrentId({ state, name: "projectId" })
    return await services.projects.addProjectAgentSessionCategory(
...
export const deleteProjectAgentSessionCategory = createAsyncThunk<
  void,
  { categoryId: string },
  ThunkConfig
>(
  "projects/deleteProjectAgentSessionCategory",
  async ({ categoryId }, { extra: { services }, getState }) => {
    const state = getState()
    const organizationId = getCurrentId({ state, name: "organizationId" })
    const projectId = getCurrentId({ state, name: "projectId" })
    await services.projects.deleteProjectAgentSessionCategory({
      organizationId,
      projectId,
      categoryId,
    })
