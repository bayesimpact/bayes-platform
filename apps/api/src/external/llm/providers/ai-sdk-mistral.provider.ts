...
@Injectable()
export class AISDKMistralProvider extends AISDKLLMProviderBase {
  getAgentProvider(): AgentProvider {
    return AgentProvider.Mistral
  }
  private readonly providerName: string

  constructor() {
    super()
    this.providerName = "Mistral Provider"
  }
  getLanguageModel({
    config,
    callOrigin,
  ...
  override applySpecificToSystemPrompt({
    ...
    if (callOrigin === CallOrigin.streamChatResponse_withTools && config.tools) {
      return MistralPromptHelper.appendToolsToPrompt({ prompt: systemPrompt, tools: config.tools })
    }
