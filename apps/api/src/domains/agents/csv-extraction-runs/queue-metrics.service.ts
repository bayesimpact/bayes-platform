...
@Injectable()
export class AgentCsvExtractionRunQueueMetricsService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(AgentCsvExtractionRunQueueMetricsService.name)
  ...
  private lastCountsByQueue: Map<string, QueueCounts> = new Map([
    [AGENT_CSV_EXTRACTION_RUN_EXECUTE_QUEUE_NAME, { ...EMPTY_COUNTS }],
    [AGENT_CSV_EXTRACTION_RUN_QUEUE_NAME, { ...EMPTY_COUNTS }],
  ])
  ...
  private async collectQueueMetrics(): Promise<void> {
    const queues: Array<[string, Queue]> = [
      [AGENT_CSV_EXTRACTION_RUN_EXECUTE_QUEUE_NAME, this.executeQueue],
      [AGENT_CSV_EXTRACTION_RUN_QUEUE_NAME, this.recordQueue],
    ]
