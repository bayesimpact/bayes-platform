...
describe("buildSubAgentTools", () => {
  ...
  it("activates a handoff-mode sub-agent via activateHandoffChild instead of running it inline", async () => {
    const childAgent = {
      id: "handoff-child-agent-id",
      projectId: "project-id",
      organizationId: "organization-id",
      name: "Specialist",
      type: "conversation",
    ...
    const activateHandoffChild = jest.fn().mockResolvedValue(undefined)

    const { tools } = await buildSubAgentTools({
      agentSessionScope: {
        agent: parentAgent,
        agentSettings: { id: "parent-agent-id-settings" },
      ...
      buildLLMConfig: (params) => params as never,
      conversationAgentSessionsService: { activateHandoffChild } as never,
      agentSettingsService: {} as never,
      buildTools: async () => {
        throw new Error("handoff mode must not build/run the child's own tools inline")
      },
      generateMasterPrompt: () => "system prompt",
      getProviderForModel: () => {
        throw new Error("handoff mode must not call the child's own LLM")
      },
      onExecute: () => {},
      projectsService: {
        hasFeature: jest.fn().mockResolvedValue(true),
    ...
    expect(tools.ask_specialist).toBeDefined()
    const handoffTool = tools.ask_specialist as never as {
      execute: (input: Record<string, never>) => Promise<{ status: string }>
    }
    const result = await handoffTool.execute({})

    expect(activateHandoffChild).toHaveBeenCalledWith({
      connectScope: { organizationId: "organization-id", projectId: "project-id" },
      parentSession,
      childAgentId: childAgent.id,
    })
