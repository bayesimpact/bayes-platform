...
import { EvaluationConversationDatasetPolicy } from "./evaluation-conversation-dataset.policy"

@Injectable()
export class EvaluationConversationDatasetGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const policy = new EvaluationConversationDatasetPolicy(
      requestToProjectPolicyContext(request),
      request.evaluationConversationDataset,
    )
