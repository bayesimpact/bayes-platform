...
| [useMount](../../apps/web/src/common/hooks/use-mount.ts) | Inside every sub-route that owns data. Dispatches `actions.mount` on mount, `actions.unmount` on unmount. | Pass `{ actions, condition? }`. Use `condition` for dependent loads (e.g. only mount when `:reviewCampaignId` is set). |
