...
The unprefixed paths predate versioning and are frozen on the shape they served then: no MCP App HTML route, no `bannerText` in the config, and a stream failure ends as a bare `event: error` frame. Additions to v1 never reach them. They exist for one integrator that has not migrated yet and are deleted, with `legacy/`, `PublicChatLegacyRoutes` and `LegacyEmbedPublicConfigDto`, once that integrator calls `/public/v1/...`. They are deliberately absent from the published documentation, which describes v1 only.
...
4. A rename, a removal, a type change, an optional element made required, a status code, error message or framing change is a breaking change. It needs a new major version under a new path prefix (`/public/v2/...`). The previous major, and with v1 the unprefixed paths, stays served for the deprecation period published on the help site.
...
4. **One server folder per served contract.** Create `apps/api/src/domains/public-chat/v2/` with its controller, its mappers and its e2e tests, mirroring `v1/`. The v2 mappers import only from the v2 contracts folder, the v1 mappers only from v1. Nothing is shared but the injected service, which returns entities, never DTOs.
5. **The shared layer is not versioned.** `PublicChatService`, the guards, the CORS branch and the SSE framing serve every major at once. Stream failures are one `{ type: "error" }` event on a 200 response, never a JSON error nor a bare `event: error` frame; that framing is part of v1, and `legacy/` keeps the older bare-frame behaviour on purpose. A change there for v2 is a change to v1 too: keep `v1/e2e-tests/` green and untouched, and treat any needed edit there as a v1 contract change under the rule above.
